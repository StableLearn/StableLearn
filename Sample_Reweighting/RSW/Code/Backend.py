...
class MpModel (nn.Module):
    def __init__(self, input_dim, output_dim=1, sigma=0.2, lam=0.005, alpha=80., hard_sum = 10.0, bias_term = True, penalty='Ours'):
        super(MpModel, self).__init__()
        self.backmodel = LinearRegression(input_dim, output_dim, bias_term)
    ...
    def pretrain(self, envs, pretrain_epoch=100):
        ...
        for i in range(pretrain_epoch):
            ...
            loss = w.reshape (1, -1).mm ((pred - y.reshape(pred.shape)) ** 2)
            loss.backward()
            pre_optimizer.step()
    ...
    def single_iter_mip(self, envs):
        ...
        output_x = self.featureSelector (x).clone().detach()
        for [x,y,w] in envs:
    ...
    def test (self, envs) :
        ...
        for [x, y] in envs :
            pred = self.single_forward (x)
            loss = self.mseloss (pred, y.reshape (pred.shape))
            loss_avg += loss / num_envs
        return loss_avg
